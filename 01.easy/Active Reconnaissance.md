...
80. Затем мы выполнили команду get для страницы по умолчанию с помощью ; мы указываем целевому серверу, что наш 
    ...
    строке ; вы можете назвать свой хост как угодно, так как это не повлияет на это упражнение.GET / HTTP/1.1host: netcat   
...
вариант	значение
-л	Режим прослушивания
-п	Укажите номер порта
-н	Только числовые данные; разрешение имен хостов через DNS не поддерживается
-в	Подробный вывод (необязательно, но полезно для обнаружения ошибок)
-вв	Очень многословно (необязательно)
-к	Продолжайте слушать после отключения клиента
Примечания:
