...
--------------------------------------------------------------------
таки образом мы имеем дело с сайтом по 80 порту, просканируем его командой:
```commandline
...
В данном случае нас конкретно интерисует только адрес http://10.10.175.180/spip/, который указывает на специфическую 
библиотеку spip.
...
после этого на нашей kali linux запустим порт 4444 для прослушивания внешних подключений через netcat или соращенно nc
```commandline
nc -lvnp 4444
```
...
-----------------------------------------------------------------------
загуглим как это сделать запросом google - bash reverse shell opnline
перейдем по ссылке:
...
перейдем на страницу https://gchq.github.io/CyberChef/ и в окне поиска введем необходиму кодировку
```commandline
to base64
```
...
для того что бы ключик заработал установим для него соотв. прова и подключимся по ssh:
```commandline
chmod 600 id_rsa
...
меня заинтерисовал вот этот файлик:
```commandline
/usr/sbin/run_container
```
